...
const IssueTimeAnalysis: React.FC<IssueTimeAnalysisProps> = ({ issues }) => {
  ...
  const filteredIssues = issues.filter((issue) => {
    ...
    const hasTimeData =
      (issue.timeEstimate || 0) > 0 || (issue.totalTimeSpent || 0) > 0;
  ...
  const categoryComparison = Object.values(CATEGORIES).map((category) => {
    ...
    const totalEstimated = categoryIssues.reduce(
      (sum, issue) => sum + issue.timeEstimate,
      0
    );
    const totalActual = categoryIssues.reduce(
      ...
      <div className="mb-8">
        ...
        <ResponsiveContainer width="100%" height={CHART_CONFIG.HEIGHTS.SMALL}>
          <BarChart data={averageDeviationPerCategory}>
            ...
            />
            <Bar
              dataKey="averageDeviation"
              fill="#ffc658"
              name="Durchschnittliche Abweichung (%)"
            />
          </BarChart>
        </ResponsiveContainer>
      </div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span
                      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        issue.timeEstimate === 0
                          ...
                          : Math.abs(issue.difference) <=
                            DEVIATION_THRESHOLDS.EXCELLENT
                          ? "bg-green-100 text-green-800"
                          : Math.abs(issue.difference) <=
                            DEVIATION_THRESHOLDS.GOOD
                    ...
                    >
                      {issue.timeEstimate === 0
                        ...
                        : `${
                            issue.difference > 0 ? "+" : ""
                          }${issue.difference.toFixed(
                            CHART_CONFIG.DECIMAL_PLACES
                          )}%`}
                    </span>
                  </td>
                </tr>
