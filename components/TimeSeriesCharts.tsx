      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        <div>
          <h4 className="text-sm font-medium text-gray-700 mb-2">
            {CHART_TITLES.WEEKLY_PERFORMANCE}
          </h4>
            ...
            <BarChart data={projectMetrics.sprintData}>
              ...
              <Tooltip
                formatter={(value: number) => [
                  ...
                  TOOLTIP_LABELS.HOURS_PER_WEEK,
                ]}
              />
              <Bar
                dataKey="totalHours"
                fill="#82ca9d"
                name={TOOLTIP_LABELS.HOURS_PER_WEEK}
              />
              <ReferenceLine
        ...
        <div>
          <h4 className="text-sm font-medium text-gray-700 mb-2">
            {CHART_TITLES.CUMULATIVE_PROGRESS}
          </h4>
          <ResponsiveContainer width="100%" height={CHART_CONFIG.HEIGHTS.SMALL}>
            <AreaChart
              data={projectMetrics.sprintData.map((sprint, index, array) => ({
            ...
            >
              ...
              <Tooltip
                formatter={(value: number | string) => [
                  `${Number(value).toFixed(CHART_CONFIG.DECIMAL_PLACES)}h`,
                  TOOLTIP_LABELS.TOTAL_HOURS,
                ]}
              />
              <Area
                type="monotone"
        ...
        <div>
          <h4 className="text-sm font-medium text-gray-700 mb-2">
            {CHART_TITLES.TEAM_ACTIVITY_COMPARISON}
          </h4>
          <ResponsiveContainer width="100%" height={CHART_CONFIG.HEIGHTS.SMALL}>
            <BarChart data={projectMetrics.sprintData}>
              <CartesianGrid strokeDasharray="3 3" />
              ...
              <YAxis />
              <Tooltip
                labelFormatter={(value: unknown) => `Sprint ${String(value)}`}
                formatter={(value: number | string) => [
                  `${Number(value).toFixed(CHART_CONFIG.DECIMAL_PLACES)}h`,
        ...
        <div>
          <h4 className="text-sm font-medium text-gray-700 mb-2">
            {CHART_TITLES.HOURS_PER_WEEK_PER_PERSON}
          </h4>
          <ResponsiveContainer
            width="100%"
              ...
              data={(() => {
                return [...lineData].sort((a, b) => {
                  ...
                  return (
                    parseInt(yearA) * 100 +
                    parseInt(weekA) -
                    (parseInt(yearB) * 100 + parseInt(weekB))
                  );
                });
              })()}
            >
        ...
        <div>
          ...
          <ResponsiveContainer width="100%" height={CHART_CONFIG.HEIGHTS.SMALL}>
            <AreaChart data={projectMetrics.sprintData}>
              ...
              <Legend />
              {Object.keys(
                projectMetrics.sprintData[0]?.categoryHours || {}
              ).map((category, index) => (
                <Area
                  key={category}
                  type="basis"
